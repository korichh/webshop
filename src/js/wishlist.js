const wishlist_init = function () {
    ...
    if (form) {
        const addToWishlistEvent = new CustomEvent('add_to_wishlist', { "detail": { target: form } })
    ...
    const updateToolbarWishlist = () => {
        ...
        for (const button of cartButtons) {
            const addToCartEvent = new CustomEvent('add_to_cart', { "detail": { target: button, wishlist: wishlist } })
    ...
    const openToolbar = (selector) => {
        ...
        setTimeout(() => {
            for (const section of sections) if (section.classList.contains('_active')) section.classList.remove('_active')
